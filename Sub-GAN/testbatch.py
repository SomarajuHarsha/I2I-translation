...
parser.add_argument('--checkpoint', type=str, help="checkpoint of style gen")
parser.add_argument('--checkpoint2', type=str, help="checkpoint of autoencoders")
...
trainer.to(device)
trainer.eval()
encode = trainer.enc_a.encode if opts.a2b else trainer.enc_b.encode # encode function
decode = trainer.enc_b.decode if opts.a2b else trainer.enc_a.decode # decode function
...
style_fixed = Variable(torch.randn(opts.num_style, style_dim, 1, 1).to(device), volatile=True)
for i, (images, names) in enumerate(zip(data_loader, image_names)):
    if opts.compute_CIS:
        cur_preds = []
    print(names[1])
    images = Variable(images.to(device), volatile=True)
    content, s = encode(images)
